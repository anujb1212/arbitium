...
export function matchMarketBuyOrder(params: {
    input: PlaceMarketInput;
    startingRemainingQty: Qty;
    asksByPrice: Map<Price, PriceLevel>;
    askPricesAsc: Price[];
    ordersById: Map<OrderId, { qtyRemaining: Qty }>;
    ...
    while (remainingQty > 0n) {
        ...
        makerOrder.qtyRemaining -= fillQty;
        ...
        if (makerOrder.qtyRemaining === 0n) ordersById.delete(makerOrder.orderId);
        prunePriceLevel({
            levelByPrice: asksByPrice,
            priceLadder: askPricesAsc,
            price: bestAskPrice
...
export function matchMarketSellOrder(params: {
    input: PlaceMarketInput;
    startingRemainingQty: Qty;
    bidsByPrice: Map<Price, PriceLevel>;
    bidPricesDesc: Price[];
