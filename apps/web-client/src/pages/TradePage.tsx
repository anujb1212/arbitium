...
const GRID_TEMPLATE_COLUMNS_CLOSED = '52px 1fr 272px 300px'
const GRID_TEMPLATE_ROWS = '64px 1fr 280px 32px'
const GRID_TRANSITION = 'grid-template-columns 300ms cubic-bezier(0.4, 0, 0.2, 1)'
...
export default function TradePage(): React.JSX.Element {
    ...
    const handleMarketChange = useCallback((market: string): void => {
        setSelectedMarket(market)
        setIsSidebarOpen(false)
        resetBook(); resetFeed(); resetStats(); openOrders.reset()
        navigate(`/trade/${market}`, { replace: true })
    }, [navigate, resetBook, resetFeed, resetStats, openOrders])
    ...
    const onPlaceSubmitted = useCallback((draft: { orderId: string; side: 'BUY' | 'SELL'; price: string; qty: string }) => {
        openOrders.addOptimistic(draft)
    }, [openOrders])

    const { registerCommandId } = useMarketFeed(selectedMarket, handleEvent)

    const onPlaceAccepted = useCallback((info: { orderId: string; commandId: string }) => {
        registerCommandId(info.commandId)
        openOrders.ackAccepted(info)
    }, [openOrders, registerCommandId])

    const onPlaceFailed = useCallback((info: { orderId: string; message?: string }) => {
        openOrders.removeByOrderId(info.orderId)
    }, [openOrders])
    ...
    return (
        <div className="w-full h-[100dvh] bg-base overflow-x-auto overflow-y-hidden scrollbar-thin">
            <div
                className="bg-base text-hi font-sans text-[13px]"
                style={{
                    display: 'grid',
                    gridTemplateColumns: isSidebarOpen ? GRID_TEMPLATE_COLUMNS_OPEN : GRID_TEMPLATE_COLUMNS_CLOSED,
            ...
            >
                ...
                >
                    <div className="flex items-center h-full">
                        <div
                            style={{ width: isSidebarOpen ? '250px' : '52px', transition: 'width 300ms cubic-bezier(0.4, 0, 0.2, 1)' }}
                            className="flex items-center justify-between px-4 h-full border-r border-line flex-shrink-0 overflow-hidden"
                        >
                            <div className="flex items-center gap-3">
                                <button
                                    onClick={() => navigate('/')}
                                    className={`flex items-center gap-3 group active:scale-[0.98] transition-all whitespace-nowrap overflow-hidden ${isSidebarOpen ? 'w-auto opacity-100' : 'w-0 opacity-0'}`}
                                >
                            ...
                            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="text-lo hover:text-hi transition-colors flex-shrink-0">
                    ...
                    <div className="flex items-center gap-5 px-5">
                        <div className={`flex items-center gap-2 text-[10px] font-mono font-medium tracking-wide ${connected ? 'text-bull' : 'text-lo'}`}>
